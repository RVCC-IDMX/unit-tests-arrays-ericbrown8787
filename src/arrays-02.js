...
function makeWordTitleCase(word) {
  const titleCase = word.substring(0, 1).toUpperCase()
...
function makeSentenceTitleCase(sentence) {
  ...
  const formattedArray = splitSentence.map((word) => makeWordTitleCase(word));
  const formattedSentence = formattedArray.join(' ');
